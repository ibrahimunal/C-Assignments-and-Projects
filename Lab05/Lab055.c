...
void delete_record (FILE *fptr)
{
    ...
    if(test.id == 0)
    {
        puts("There is no film with id %s",temp.id);
    }
    else
        rewind(fptr);
    fseek(fptr,(id-1)*sizeof(struct film),SEEK_SET);
...
void sell_ticket (FILE *fptr){
    ...
    else
    {
        test.id = id;
        int tempint;
        printf("Enter the ID of the film: %d",id);
        puts("ID No\tFilm Name\tFormat\t  Show Date   Show Time     Price  Capacity");
        printf("%-8d%-16s%-10s%-11s%10s%10d%10d\n",test.id,test.name,test.format,test.show_date,test.show_time,test.price,test.capacity);
        printf("Enter ticket amount to sell: ");
        scanf("%d",&tempint);
        test.capacity=test.capacity-tempint;
        fseek(fptr,(id-1)*sizeof(struct film),SEEK_SET);
        fwrite(&test,sizeof(struct film),1,fptr);
